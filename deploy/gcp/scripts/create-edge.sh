...
#          request logging reconciled every run), two URL
...
for triple in "${TRIPLES[@]}"; do
  ...
  # THE BALANCER'S REQUEST LOGGING IS RECONCILED EVERY RUN. A backend service is created with
  # logging off, and `_ensure` reuses an existing one as it is - so when dev's console backend
  # started failing chat turns in the browser ("Failed to fetch"), the balancer had recorded
  # nothing, and neither had Cloud Run behind it, which never received those requests. A request
  # the edge refuses has to be visible at the edge. (The request TIMEOUT of a serverless-NEG
  # backend is not ours to set: gcloud refuses `--timeout` on one - "Timeout sec is not supported
  # for a backend service with Serverless network endpoint groups" - and the balancer applies its
  # own fixed limit, well above the consoles' CONSOLE_TIMEOUT_SECONDS.)
  gc compute backend-services update "${backend}" --global \
    --enable-logging --logging-sample-rate=1 >/dev/null
  log "  ${backend} logs every request it handles or refuses"
