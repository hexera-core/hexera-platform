# Responsibility: Implement the blocking gates a snappyHexMesh run must pass before review.
...
#: Defined once for the bundle, beside the reconstruction check that also reads it.
from meshpipeline.engines.snappy.parallel_stages import REQUIRED_POLYMESH as _POLYMESH_REQUIRED
...
def _validate_manifest(workspace: Path, domain: str = "") -> tuple[bool, str]:
    ...
    # snappyHexMesh writes a native OpenFOAM polyMesh. (The engine label here used to read
    # "cfMesh" - inherited verbatim from the retired shared module, so a snappy user was told
    # another mesher had failed.)
    if not manifest.get("mesh_written", False):
        return False, "mesh_manifest.json: snappyHexMesh reported the mesh was not written"
    ok, why = _polymesh_is_complete(workspace / "constant" / "polyMesh", "snappyHexMesh")
    if not ok:
        return False, why
    return True, ""
...
def _gate_quality_floor(ctx: GateCtx) -> tuple[bool, str]:
    ...
    # gmsh and vmtk have always had the equivalent boundary (sicn_floor / quality_floor); snappy
    ...
    return gate_declared_criteria("snappy", ctx.manifest_or_load())
