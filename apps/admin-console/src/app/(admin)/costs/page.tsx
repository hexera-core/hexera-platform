...
export default async function CostsPage() {
  ...
  const account = await readBillingAccount(getBillingReader(), targets.projectId);
  const budgets = account.name ? await readBudgets(getBudgetReader(), account.name) : [];
  ...
  return (
    <>
      ...
      <Panel heading={`${budgets.length} configured`} title="Budgets">
        {budgets.length === 0 ? (
