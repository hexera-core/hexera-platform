...
export const Stage = {
  ...
  /* THE GEOMETRY CHECK: the part with a numbered sticker on every opening it found, and
     what it proposes for each number - name, role, size, position - editable in place. The
     user fixes what is wrong and proceeds; the reply is the sentence the intake will treat
     as declared. One card per session, replaced if the check is shown again. */
  geometryCheck(d,confirm){
    const p=d.proposal||{},pics=d.pictures||[];
    const overview=pics.find(x=>x.name==='overview')||pics[0];
    const mm=v=>(v==null||isNaN(v))?'?':String(Math.round(v));
    const KIND=[['body-surface',"the part's wall, hollow inside for the fluid"],
                ['fluid-domain','the fluid volume itself'],
                ['solid-body','a solid body the fluid flows around']];
    const sel=(cls,opts,cur)=>`<select class="${cls}">${opts.map(([v,l])=>
      `<option value="${v}"${cur===v?' selected':''}>${esc(l)}</option>`).join('')}</select>`;
    const rows=(p.openings||[]).map(o=>{
      const size=o.shape==='circle'?`${mm(o.diameter_mm)} mm across`:`${mm(o.width_mm)} x ${mm(o.height_mm)} mm`;
      const at=(o.centroid_mm||[]).map(v=>mm(v)).join(', ');
      return `<tr data-id="${o.id}"><td class="gc-n">${o.id}</td>
        ...
        <td class="gc-dim">${esc(size)}</td><td class="gc-dim">(${esc(at)}) mm</td>
        <td class="gc-conf" title="how sure the check is">${Math.round((o.confidence||0)*100)}%</td></tr>`;}).join('');
    const notes=(p.notes||[]).map(n=>`<div class="gc-note">${esc(n)}</div>`).join('');
    const thumbs=pics.filter(x=>x!==overview).slice(0,8).map(x=>
    ...
    const html=`<div class="who">Geometry check · what Hexera sees</div>
      <div class="req-card gc-card">
        ...
        ${overview?`<img class="gc-overview" src="${overview.url}" alt="the part, with a numbered sticker on each opening" title="click to enlarge">`:''}
        ${thumbs?`<div class="gc-thumbs">${thumbs}</div>`:''}
        <div class="rc-row"><div class="rc-k">The file is</div><div class="rc-v">${sel('gc-sel gc-kind',KIND,p.input_kind)}</div></div>
        ...
        ${rows?`<table class="gc-table"><thead><tr><th>#</th><th>name</th><th>role</th><th>size</th><th>position</th><th>sure</th></tr></thead><tbody>${rows}</tbody></table>`
    ...
    btn.onclick=async()=>{
      const flow=g.querySelector('.gc-flow').value;
      const openings=flow==='external'?[]:[...g.querySelectorAll('tbody tr')].map(tr=>{
        ...
        return body;});
      const body={input_kind:g.querySelector('.gc-kind').value,flow,part:p.part||'',
                  openings,seed_point_mm:p.seed_point_mm||null,size_mm:p.size_mm||null};
      btn.disabled=true;btn.textContent='Confirming…';
      try{
        await confirm(body);
        g.querySelectorAll('input,select').forEach(el=>{el.disabled=true;});
        g.querySelector('.gc-actions').innerHTML='<span class="gc-done">✓ Confirmed. The questions that follow will skip all of this.</span>';
